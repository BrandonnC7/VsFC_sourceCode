...
class Controls extends FlxActionSet
{
	...
	var _powerup = new FlxActionDigital(Action.POWERUP);
	...
	public var POWERUP(get, never):Bool;

	inline function get_POWERUP()
		return _powerup.check();
	#if (haxe >= "4.0.0")
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				inline bindKeys(Control.DODGE, keysMap.get('attack'));
				inline bindKeys(Control.POWERUP, keysMap.get('power_up'));
			...
			case Duo(true):
				...
				inline bindKeys(Control.POWERUP, [ONE]);
			case Duo(false):
				...
				//inline bindKeys(Control.DODGE, [FlxKey.DODGE]);
				//inline bindKeys(Control.ATTACK, [FlxKey.ATTACK]);
				//inline bindKeys(Control.POWERUP, [FlxKey.POWERUP]);
			case None: // nothing
			case Custom: // nothing
		}
		#else
		switch (scheme)
		{
			...
			case Duo(false):
				...
				bindKeys(Control.DODGE, [FlxKey.SPACE]);
				bindKeys(Control.ATTACK, [FlxKey.SHIFT]);
				bindKeys(Control.POWERUP, [FlxKey.ONE]);
			case None: // nothing
			case Custom: // nothing
		}
		#end
