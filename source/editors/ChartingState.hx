...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var plrVoicesVolume:FlxUINumericStepper;
	var oppVoicesVolume:FlxUINumericStepper;
	function addChartingUI() {
		...
		*/
		...
		var check_mute_plrVocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y, null, null, "Mute Player Vocals (in editor)", 100);
		check_mute_plrVocals.checked = false;
		check_mute_plrVocals.callback = function()
		{
			if(plrVocals != null) {
				var plrVol:Float = 1;

				if (check_mute_plrVocals.checked)
					plrVol = 0;

				plrVocals.volume = plrVol;
			}
		}

		var check_mute_oppVocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y - 120, null, null, "Mute Opponent Vocals (in editor)", 100);
		check_mute_oppVocals.checked = false;
		check_mute_oppVocals.callback = function()
		{
			if(oppVocals != null) {
				var oppVol:Float = 1;

				if (check_mute_oppVocals.checked)
					oppVol = 0;
		...
		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_plrVocals.y + 30, null, null, 'Play Sound (Boyfriend notes)', 100,
			function() {
		...
		*/
		...
		plrVoicesVolume = new FlxUINumericStepper(instVolume.x + 100, instVolume.y, 0.1, 1, 0, 1, 1);
		plrVoicesVolume.value = plrVocals.volume;
		plrVoicesVolume.name = 'plrVoices_volume';
		blockPressWhileTypingOnStepper.push(plrVoicesVolume);

		oppVoicesVolume = new FlxUINumericStepper(instVolume.x + 100, instVolume.y, 0.1, 1, 0, 1, 1);
		oppVoicesVolume.value = oppVocals.volume;
		oppVoicesVolume.name = 'oppVoices_volume';
		blockPressWhileTypingOnStepper.push(oppVoicesVolume);
		...
		tab_group_chart.add(new FlxText(plrVoicesVolume.x, plrVoicesVolume.y - 15, 0, 'Voices Volume'));
		...
		tab_group_chart.add(plrVoicesVolume);
		tab_group_chart.add(oppVoicesVolume);
		tab_group_chart.add(check_mute_inst);
		//tab_group_chart.add(check_mute_vocals);
		tab_group_chart.add(check_mute_plrVocals);
		tab_group_chart.add(check_mute_oppVocals);
	...
	function loadSong():Void
	{
		...
		*/
		...
		var plrFile:Dynamic = Paths.playerVoices(currentSongName);
		plrVocals = new FlxSound();
		if (Std.isOfType(plrFile, Sound) || OpenFlAssets.exists(plrFile)) {
			plrVocals.loadEmbedded(plrFile);
			FlxG.sound.list.add(plrVocals);
		}

		var oppFile:Dynamic = Paths.opponentVoices(currentSongName);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'plrVoices_volume')
			{
				plrVocals.volume = nums.value;
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if(plrVocals != null) plrVocals.stop();
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				if (FlxG.sound.music.playing)
				{
					...
					if(plrVocals != null) plrVocals.pause();
					if(oppVocals != null) oppVocals.pause();
				}
				else
				{	
					if(plrVocals != null && oppVocals != null) {
						plrVocals.play();
						plrVocals.pause();
						plrVocals.time = FlxG.sound.music.time;
						plrVocals.play();
		...
		plrVocals.pitch = playbackSpeed;
		oppVocals.pitch = playbackSpeed;
